...
export const TestEditable = props => {
  const { test: {title, code, status, hz, rme, fastest, slowest, percent}, id, onUpdate } = props
    ...
    <tr className="">
      ...
      <td className="py-5 px-2 bg-gray-200 w-1/6 border border-slate-300">
        <div className="flex relative w-500px h-48px group justify-center items-center z-1001">
          ...
          <input type="text" className="p-2 bg-transparent border-b-2 hover:border-blue-500 focus:outline-none focus:border-blue-500 border-gray-200" placeholder={`Test ${id + 1}`} defaultValue={title} />
      ...
      <td className="code border border-slate-300">
        <Editor code={code} onUpdate={onUpdate} className="javascript w-full h-full p-2 block" style={{minHeight: "100px"}} />
        {/* <pre className="w-full whitespace-pre-wrap break-words"> */}
        {/*   <code dangerouslySetInnerHTML={ */}
        {/*     {__html: highlightSanitizedJS(code)}} /> */}
        {/* </pre> */}
